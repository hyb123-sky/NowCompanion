...
| A10a | Reference deployment path — image builds and runs | CI (`deploy-reference` job, `ubuntu-latest`, Docker available natively): build the image from `/deploy/reference`, start it via the compose file, hit the health endpoint, assert a response, tear down. Continuous evidence on every PR touching `/deploy/reference` or the Gateway, not a one-off local check. | **Verified, with a negative control, on PR #7.** A green check that never goes red proves nothing (same discipline as A1's mutation test). Sequence: green on the PR's original head (run [34444520216](https://github.com/hyb123-sky/NowCompanion/actions/runs/34444520216), `deploy-reference`, success) → a deliberate `/healthz` breakage (commit `edc312a`) went red (run [34447344973](https://github.com/hyb123-sky/NowCompanion/actions/runs/34447344973), `deploy-reference`, failure — `gateway-build-test` also correctly failed the same commit, run [34447344904](https://github.com/hyb123-sky/NowCompanion/actions/runs/34447344904), unit-level `HealthzTests.Healthz_Returns_Ok` catching the same regression) → the revert (commit `df7fc28`) went green again (run [34447546237](https://github.com/hyb123-sky/NowCompanion/actions/runs/34447546237), `deploy-reference`, success). An earlier break/revert pair (commits `062eb09`/`7e72305`) produced **no CI evidence at all** — both were pushed before PR #7 existed, so `pull_request` never ran against either; do not mistake those commits for the control. Proves: image builds, container starts, health endpoint responds, and the check can actually detect a real regression. Does **not** prove A10b. |
